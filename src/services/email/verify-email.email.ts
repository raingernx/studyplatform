          ...
          <tr>
            <td style="padding:32px;">
              ...
              <p style="margin:0 0 28px;font-size:14px;color:#6b7280;line-height:1.6;">
                Confirm your email so we can send receipts, downloads, and account recovery links to the right inbox.
              </p>
              <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="margin-bottom:18px;">
                <tr>
                  <td>
                    <a href="${input.verifyUrl}" style="display:block;text-align:center;background:#18181b;color:#ffffff;text-decoration:none;font-size:15px;font-weight:600;padding:14px 24px;border-radius:12px;">
                      Verify email
                    </a>
                  </td>
                </tr>
              ...
              <p style="margin:0;font-size:12px;color:#9ca3af;line-height:1.7;">
                This link expires in 24 hours. If you created this account by mistake, no further action is needed.
              </p>
            </td>
          </tr>
        </table>
...
export async function sendVerificationEmail(input: {
  ...
  try {
    ...
    if (error) {
      console.error("[EMAIL] Verification email rejected:", error);
    }
  } catch (error) {
