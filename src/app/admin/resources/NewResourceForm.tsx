...
export function NewResourceForm({ categories, tags }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // [2] Throw so the single catch block handles all error paths
  ...
  return (
    <div>
        ...
        {urls.map((url, i) => (
          <div key={i} className="flex items-center gap-2">
            ...
            <button
              type="button"
              onClick={() => remove(i)}
              className="shrink-0 rounded-lg p-1.5 text-zinc-400 transition
      ...
      <button
        type="button"
        onClick={add}
        className="mt-2 inline-flex items-center gap-1.5 rounded-xl border border-dashed
                   border-zinc-300 px-4 py-2 text-[12px] font-medium text-zinc-500
...
function FilePickerSection({
  ...
  return (
    <div>
      ...
      {file && (
        <div className="mb-2 flex items-center gap-3 rounded-xl border border-blue-100 bg-blue-50 px-4 py-3">
