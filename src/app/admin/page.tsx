...
import { AdminDashboardRecentActivitySkeleton } from "@/components/skeletons/AdminCoreRouteSkeletons";
...
async function AdminRecentActivitySection({
  dataPromise,
}: {
  dataPromise: ReturnType<typeof getAdminDashboardRecentActivity>;
}) {
  const { recentPurchases, recentResources } = await dataPromise;
...
export default async function AdminDashboardPage() {
  return withRequestPerformanceTrace("route:/admin", { routeKind: "overview" }, async () => {
    const metricsPromise = traceServerStep("admin_overview.getAdminDashboardMetrics", () =>
      getAdminDashboardMetrics(),
    );
    const recentActivityPromise = traceServerStep(
      "admin_overview.getAdminDashboardRecentActivity",
      () => getAdminDashboardRecentActivity(),
    );
    ...
    return (
      <div data-route-shell-ready="admin-overview" className="space-y-7">
        ...
        <Suspense fallback={<AdminDashboardRecentActivitySkeleton />}>
          <AdminRecentActivitySection dataPromise={recentActivityPromise} />
        </Suspense>
      </div>
    );
  });
