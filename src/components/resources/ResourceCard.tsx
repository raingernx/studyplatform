...
function CardBody({
  ...
  const thumbWrapperClass = `relative aspect-[4/3] w-full overflow-hidden rounded-t-xl rounded-b-none bg-muted${
    isMarketplace ? " h-[244px]" : ""
  }`;

  const articleClass =
    isMarketplace
      ? "flex h-full w-full flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-sm transition-all duration-200 hover:-translate-y-[2px] hover:shadow-md"
      : isHero
        ? "flex h-full w-full flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-[0_4px_12px_rgba(0,0,0,0.06)]"
        : "flex h-full w-full flex-col overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-[0_4px_12px_rgba(0,0,0,0.06)]";
    ...
    <article className={articleClass}>
      ...
      <div className="flex flex-1 flex-col space-y-2 p-4">
        ...
        <div className="flex items-start justify-between gap-3">
          <h3
            className={`text-sm font-medium leading-snug tracking-[0.1px] text-neutral-900 ${
              isMarketplace ? "line-clamp-1" : "line-clamp-2"
            }`}
          >
            {resource.title}
          </h3>
