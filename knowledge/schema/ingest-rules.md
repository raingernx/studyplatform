...
- add `--report-format bundle` when the report artifact should include both a human-readable `textSummary`, path-oriented `artifacts` / `annotations`, CI-ready `ciHints`, and structured sections alongside the raw `plan`; `ciHints` now includes `githubStepSummary` and GitHub-friendly `annotations[]` entries. Omit the flag to keep the legacy plan-only JSON artifact
