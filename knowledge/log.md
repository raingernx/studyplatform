...
- removed the duplicate standalone internal cache warm step from the post-deploy workflow after the warm script itself had already learned to call `/api/internal/performance/warm` before route fanout. Keeping the extra workflow step after route warm meant the perf suite no longer started immediately after `/resources` and listing shells were last heated, which was the wrong order for the routes the smoke suite measures first.
