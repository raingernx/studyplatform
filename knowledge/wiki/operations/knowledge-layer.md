...
- dry-run preview commands now also support `--format json`, which emits the same merge plan as machine-readable JSON for agent orchestration or CI automation, including per-item/per-target `decision` hints with `actions`, `reasons`, `severity`, and a top-level `decisionSummary`.
