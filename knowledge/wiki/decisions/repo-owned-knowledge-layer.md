...
- dry-run preview can now emit machine-readable JSON with `--format json`, including per-item/per-target `decision` hints plus a top-level `decisionSummary`, and now also `confidence` / `policy` hints plus batch-level policy overrides so agent/CI workflows can consume merge plans without screen-scraping CLI text or inferring create/update/merge/apply intent from raw structure alone. When a workflow wants policy to be hard-blocking instead of advisory, `--enforce-policy` now exits non-zero only for `blocked_by_policy`, write mode honors the same gate before touching files, `--report-file` can persist the blocked plan as a JSON artifact, and `--report-format bundle` upgrades that artifact into a single file with `textSummary`, path-level `artifacts`, review `annotations`, CI-ready `ciHints`, GitHub-step-summary content, GitHub-friendly `annotations[]`, structured sections, and the raw plan.
