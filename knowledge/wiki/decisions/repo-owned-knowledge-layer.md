...
- dry-run preview can now emit machine-readable JSON with `--format json`, including per-item/per-target `decision` hints plus a top-level `decisionSummary`, and now also `confidence` / `policy` hints so agent/CI workflows can consume merge plans without screen-scraping CLI text or inferring create/update/merge/apply intent from raw structure alone.
