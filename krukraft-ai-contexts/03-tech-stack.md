...
- dry-run ingest also supports `--format json`, and the repo now exposes `wiki:ingest:dry-run:json` plus `wiki:ingest:batch:dry-run:json` helpers for machine-readable plan export; those JSON previews now carry per-item/per-target `decision` hints (`actions`, `reasons`, `severity`), a top-level `decisionSummary`, `confidence` / `policy` metadata for apply-vs-review gating, and top-level batch `policy` overrides for CI-enforced review thresholds
