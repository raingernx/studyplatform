...
- `.github/workflows/browser-smoke.yml` now also boots a fresh `next dev` instance after the Playwright smoke bundle and runs `npm run browser:probe:dashboard`, so dashboard route transitions have a separate cloud browser probe path even when local macOS Playwright launch is unreliable
