...
- `resources_home_smoke`
  ...
  - current mitigation: `/resources` now warms as a required route with `repeat: 3` and `burst: 5`, then closes the final tail band with another `repeat: 2` and `burst: 5` so the discover-home shell is not cooled behind later creator/category reheats before k6 starts
- `listing_newest_smoke`
