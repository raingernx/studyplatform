...
- the dashboard overview, settings, subscription, library, downloads-history, purchases-history, and legacy purchases-redirect routes now bypass the DB-backed creator-access shell blocker entirely by living under a dedicated dashboard-lite route group whose layout derives creator shell state from a signed JWT token snapshot first instead of awaiting `getCreatorAccessState()` or the Prisma-sensitive `getServerSession()` role-refresh path before first shell render
- `/dashboard/library`, `/dashboard/downloads`, and `/dashboard/purchases` no longer block their full page shell on purchased-item queries; those dashboard-lite routes now render the route shell after `requireSession()` and stream their heavy results branches behind structural `Suspense` fallbacks
...
- the dashboard settings route now renders its section shell before preferences/profile data resolve and uses a tabs-only fallback that matches the final geometry, so authenticated settings visits no longer wait on the full settings payload before any in-page shell HTML appears
- `/settings` and `/subscription` are now protected at the proxy layer as top-level dashboard-lite routes, which removes the old page-level auth gate from the first shell render path and prevents dashboard entry overlays from waiting on a redirect check before any route-ready marker can appear
...
- `/settings` and `/subscription` now also resolve their hot user snapshot from the signed JWT token before falling back to `getServerSession()` inside the page branch, removing the second NextAuth session callback hop from the common account-page path
- public account-menu transitions into `/settings` and `/subscription` no longer request the full dashboard entry overlay; those two top-level dashboard-lite account routes now navigate as non-overlay dashboard handoffs, and `DashboardEntryNavigationOverlay` only keeps a forced full-screen dashboard-group skeleton for transitions whose navigation state explicitly asked for one
- the dashboard-lite `/dashboard`, `/settings`, `/subscription`, `/dashboard/library`, `/dashboard/downloads`, `/dashboard/purchases`, and `/purchases` routes keep the same dashboard loading/overlay family markers (`dashboard`, plus their route-specific markers) while skipping the creator-access DB wait at layout entry, so dashboard handoff probes still target the same shell selectors even though those routes no longer live under `src/app/(dashboard)`
- the `/subscription` results fallback now uses the same hero/stat/billing geometry as the final membership surface instead of a generic stacked placeholder, reducing visible skeleton shift on the membership route while its streamed branch resolves
...
- dashboard-shell geometry is now standardized across user and creator pages: route headers use one shared contract, and dashboard-lite routes now keep the same `space-y-8` shell rhythm across overview, library, downloads, purchases, settings, and membership so loading handoff is less route-specific
...
- dashboard-family audit follow-up (2026-04-09)
  - the outer `(dashboard)` shell no longer pays the old DB-backed session/access path by default on every request; it now resolves shell identity from the signed JWT token snapshot first and only falls back to `getServerSession()` / `getCreatorAccessState()` when the token cannot answer the shell question on its own
  - dashboard entry overlays now clear from one shared navigation-state path instead of overlapping local clear loops in the root entry overlay, the in-dashboard overlay, and the dashboard shell itself; `DashboardEntryNavigationOverlay` owns cross-group dashboard handoff, while `DashboardGroupNavigationOverlay` now only covers resource overlays within dashboard routes
...
- management long-tail shell audit (2026-04-09)
  ...
  - `/dashboard/resources` now fast-paths the compatibility redirect from the session role for `ADMIN`/`INSTRUCTOR` users and only hits `getCreatorAccessState()` for the remaining users, trimming the common creator/admin redirect path without weakening the actual creator workspace gate
...
14. Compatibility/onboarding routes should use role fast-paths before falling back to creator-access reads; `/dashboard/resources` and `/dashboard/creator/apply` now both short-circuit for clearly admitted roles, so if those paths regress again inspect whether the fast-path was removed before blaming creator-access cache latency
