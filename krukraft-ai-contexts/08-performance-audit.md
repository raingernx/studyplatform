...
- the below-the-fold discover collections subtree on `/resources` now runs behind a short best-effort timeout guard, so cold collection reads can drop out instead of stretching the entire resources-home response when the shell and lead rows are already ready
...
- `resources_home_smoke`
  ...
  - current mitigation: `/resources` still keeps the required warm burst and tail reheat, but the main app-level mitigation now is route decomposition itself: quick-browse shell renders immediately while discover `lead` and `collections` stream from separate cached readers instead of waiting on a monolithic discover payload, the default `Top picks` / `Trending now` rows now use a server-led public card shell instead of the generic client card wrappers, and the slower below-the-fold collections subtree now has a short best-effort timeout so it cannot hold the whole response open
- `listing_newest_smoke`
  ...
  - current mitigation: retain `burst: 5`, keep repeated warm passes, reheat the listing route again as one of the last warm steps immediately before k6 begins, and prime a dedicated fixed-key landing cache for the hot `sort=newest&page=1&category=all` shape so the warmed smoke route and runtime landing path hit the same Redis surface
- `listing_recommended_smoke`
