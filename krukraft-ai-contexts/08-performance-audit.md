...
- the dashboard overview, settings, subscription, library, downloads-history, purchases-history, and legacy purchases-redirect routes now bypass the DB-backed creator-access shell blocker entirely by living under a dedicated dashboard-lite route group whose layout derives creator shell state from the cached session role instead of awaiting `getCreatorAccessState()` before first shell render
...
- the dashboard avatar-menu sentinel now targets explicit dashboard menu data attributes instead of layout/CSS selectors, so CI failures in that lane reflect a real menu contract break instead of a fragile probe coupling to absolute-positioned DOM
