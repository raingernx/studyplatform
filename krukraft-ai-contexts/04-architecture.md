...
src/app/
  ...
  (dashboard-v2)/dashboard-v2/* ← Canonical protected dashboard family
  admin/*                     ← Admin panel
  api/*                       ← Thin route handlers
```

Historical note:
- the older `(dashboard)` and `(dashboard-lite)` route groups were retired in
  the Phase 5 hard cut and are no longer part of the live route structure
...
- the proxy no longer canonicalizes old dashboard-lite or creator dashboard routes; `/dashboard-v2/*` is the only protected dashboard route family after the hard cut
- `/dashboard-v2*` is covered directly by the proxy-level dashboard guard alongside `/admin`
- `/dashboard-v2` home is no longer a public prototype surface: the page now calls `requireSession(routes.dashboardV2)`, redirects anonymous visits to `/auth/login?next=%2Fdashboard-v2`, and resolves a data-backed learner home summary through `src/services/dashboard-v2/home.service.ts`
- `/dashboard-v2/library` is now the second integrated Dashboard V2 learner route: it also calls `requireSession(routes.dashboardV2Library)`, resolves owned-library data through `src/services/dashboard-v2/library.service.ts`, and treats `q`, `filter`, and checkout-return recovery as route-owned state
- `/dashboard-v2/downloads` is now the third integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Downloads)`, resolves download history through `src/services/dashboard-v2/downloads.service.ts`, and keeps re-download entry points pinned to `/api/download/:resourceId` instead of exposing raw file URLs in the route body
- `/dashboard-v2/purchases` is now the fourth integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Purchases)`, resolves order history through `src/services/dashboard-v2/purchases.service.ts`, and keeps purchase status/reference rendering in the route body separate from protected file-delivery entry points
- `/dashboard-v2/membership` is now the fifth integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Membership)`, resolves subscription status/renewal/billing snapshot data through `src/services/dashboard-v2/membership.service.ts`, and now renders the learner account shell/intro before streaming the membership results body through an in-page `Suspense` boundary. Plan CTA decisions stay route-owned without exposing billing internals outside the authenticated surface.
- `/dashboard-v2/settings` is now the sixth integrated Dashboard V2 learner route: it calls `requireSession(routes.dashboardV2Settings)`, resolves a read-first settings payload through `src/services/dashboard-v2/settings.service.ts`, and renders real interactive `ProfileSettings`, `PreferenceSettings`, and `NotificationSettings` sections inside the dashboard-v2 shell instead of a read-only account summary. The dashboard-v2 settings surface no longer exposes a language selector; the route now treats Thai as the fixed product language and keeps only theme, currency, timezone, notifications, account-access controls, and a real profile-photo update path on-page. Profile photos now have a provider-backed fallback contract too: Google sign-in persists the original provider image into `User.providerImage`, custom uploads replace only `User.image`, and removing a custom photo from settings restores the stored Google avatar when one exists instead of dropping straight to initials.
- `/dashboard-v2/creator` is now data-backed too: it resolves creator access through `requireSession(routes.dashboardV2Creator)` and `src/services/dashboard-v2/creator-overview.service.ts`, and renders the creator workspace/profile hub inside the dashboard-v2 shell instead of falling back to static creator prototype panels.
- `/dashboard-v2/creator/analytics` is now data-backed: it calls `requireSession(routes.dashboardV2CreatorAnalytics)`, resolves creator metrics/top resources/recent sales/recent downloads through `src/services/dashboard-v2/creator-analytics.service.ts`, and keeps the locked/error handling route-owned instead of rendering static trend placeholders.
- `/dashboard-v2/creator/sales` and `/dashboard-v2/creator/payouts` now share the same protected earnings data contract: both routes call `requireSession(...)`, resolve creator sales + balance data through `src/services/dashboard-v2/creator-earnings.service.ts`, and render route-owned sales/payout history tables instead of the earlier prototype panels.
- `/dashboard-v2/creator/profile` is now data-backed: it calls `requireSession(routes.dashboardV2CreatorProfile)`, resolves creator access + profile data through `src/services/dashboard-v2/creator-profile.service.ts`, and renders the existing `CreatorProfileForm` inside the dashboard-v2 shell instead of routing back through the creator workspace summary surface. The profile form now owns creator-specific storefront media too: it can upload and save a dedicated `creatorAvatar` plus `creatorBanner` through `/api/creator/upload/image` + `/api/creator/profile`, while public creator pages and dashboard storefront/profile summaries prefer `creatorAvatar` over the account-level `user.image` fallback.
- the Dashboard V2 shell viewer contract now also carries an optional
...
- `/dashboard-v2/creator/storefront` is no longer a real dashboard destination:
  ...
  `/dashboard-v2/creator/profile` as a fallback
- `/dashboard-v2/creator/settings` is also compatibility-only now and redirects
  to shared account settings at `/dashboard-v2/settings`; creator-specific
  settings summaries no longer render inline on the workspace route
- `/dashboard-v2/creator/resources/new` and `/dashboard-v2/creator/resources/[id]` now enforce `requireSession(...)` and resolve a route-owned creator-editor contract through `src/services/dashboard-v2/creator-editor.service.ts`. Non-creator accounts land on a locked state inside the dashboard-v2 shell, invalid ids land on a not-found state, and the edit route now loads the real `CreatorResourceForm` with owner-checked resource data instead of rendering a prototype editor scaffold.
- the wider Dashboard V2 route family now has concrete route files for the learner and creator surfaces: `/dashboard-v2`, `/dashboard-v2/library`, `/dashboard-v2/downloads`, `/dashboard-v2/purchases`, `/dashboard-v2/membership`, `/dashboard-v2/settings`, `/dashboard-v2/creator`, creator resources list/create/edit, analytics, sales, payouts, profile, and settings.
- creator compatibility routing at the proxy layer was removed during the hard cut; canonical `/dashboard-v2/creator/*` routes now own live creator dashboard behavior directly.
- unauthenticated `/dashboard-v2*` and `/admin*` requests redirect to `/auth/login?next=...`
- authenticated non-admin `/admin*` requests redirect to `/dashboard-v2`
...
- the root client provider tree now mounts `SessionProvider` again alongside the design-system toast provider. Public routes still avoid page-level session reads and rely on targeted auth-viewer fetches for auth-aware chrome, but client account surfaces such as `/dashboard-v2/settings` can now call `useSession().update(...)` to refresh name/email/avatar state immediately after a successful profile save.
- the root layout keeps two cross-group overlay bridges: `ResourcesEntryNavigationOverlay` for dashboard/public → `/resources` transitions, and `DashboardEntryNavigationOverlay` for public → `/dashboard-v2/*` transitions. The dashboard entry overlay is intentionally shell-only (`DashboardV2ShellSkeleton`) so it reserves only navbar + sidebar while the destination dashboard route mounts its own route-owned content `loading.tsx`.
...
- the dashboard skeleton layer now mirrors that same page-shell contract instead of hand-rolling per-file outer wrappers; settings, membership, dashboard-v2 learner pages, creator overview/analytics/resources/sales/profile, and creator resource editor fallbacks all render through the same shell primitive before their route-specific sections take over
...
- the active dashboard loading contract is now dashboard-v2-only: `src/app/(dashboard-v2)/dashboard-v2/layout.tsx` owns the real dashboard chrome, while each canonical route `loading.tsx` renders only content skeletons inside that shell. The learner-family parent fallback at `src/app/(dashboard-v2)/dashboard-v2/loading.tsx` is now intentionally invisible (`null`), and `/dashboard-v2` overview keeps its home-shaped loading ownership in `src/app/(dashboard-v2)/dashboard-v2/(overview)/loading.tsx` so sibling learner routes can jump directly to their own route fallback without a visible family-level neutral skeleton. The creator-family parent fallback at `src/app/(dashboard-v2)/dashboard-v2/creator/loading.tsx` is now also intentionally invisible (`null`), and `/dashboard-v2/creator` workspace keeps its workspace-shaped loading ownership in `src/app/(dashboard-v2)/dashboard-v2/creator/(workspace)/loading.tsx` so creator children such as resources/sales/payouts can jump directly to their own route fallback without a visible creator-family neutral skeleton. The old `(dashboard)` / `(dashboard-lite)` route groups and `DashboardGroupLoadingShell` were retired during the Phase 5 hard cut.
...
- route-level dashboard-v2 `loading.tsx` files remain the preferred owners for visible route bodies whenever a learner or creator page needs a substantive loading state. Page-level `Suspense` should be reserved for secondary subsections instead of owning the main route body.
- the staged Dashboard V2 route family lives under `src/app/(dashboard-v2)/dashboard-v2/*` and keeps a shell-owned loading contract: `layout.tsx` owns the static dashboard-v2 shell, top-level learner and creator family fallbacks stay neutral, and route `loading.tsx` files render content-only skeletons inside that shell. `/dashboard-v2` home now uses an isolated `(overview)` route group and `/dashboard-v2/creator` workspace now uses an isolated `(workspace)` route group so those overview surfaces can keep their own page-shaped loading without reintroducing cross-route home-first or workspace-first flashes on sibling transitions, while the heavier `DashboardV2PrototypeSkeleton` remains a route-family scaffold for first-entry/full-surface preview work rather than a child-route fallback.
- the Dashboard V2 shell now also resolves a minimal optional viewer snapshot from `getCachedServerSession()` and passes only display-name/email/image/role/subscription-status plus optional `creatorPublicHref` into the shell chrome. That shell snapshot remains separate from route-level auth ownership: `/dashboard-v2`, `/dashboard-v2/library`, `/dashboard-v2/downloads`, `/dashboard-v2/purchases`, `/dashboard-v2/membership`, `/dashboard-v2/settings`, `/dashboard-v2/creator`, `/dashboard-v2/creator/resources`, `/dashboard-v2/creator/resources/new`, `/dashboard-v2/creator/resources/[id]`, `/dashboard-v2/creator/analytics`, `/dashboard-v2/creator/sales`, `/dashboard-v2/creator/payouts`, and `/dashboard-v2/creator/profile` enforce their own `requireSession(...)` gates while the creator compatibility routes `/dashboard-v2/creator/storefront` and `/dashboard-v2/creator/settings` only validate enough to redirect safely
- Phase 5 cutover has hard-cut the learner-facing route family to `dashboard-v2`: active user dashboard nav, auth callbacks, purchase CTAs, and shared menus point directly at `/dashboard-v2/*` destinations instead of old `/dashboard*`, `/settings`, or `/subscription` surfaces.
- Proxy-owned old URL canonicalization has been removed after confirming the project has no real users yet, so browser URLs such as `/dashboard`, `/dashboard/library`, `/dashboard/downloads`, `/dashboard/purchases`, `/settings`, and `/subscription` are no longer supported bookmark redirects into dashboard-v2.
- Shared learner navigation now treats `dashboard-v2` as canonical: the marketplace navbar account menu, dashboard topbar avatar menu, dashboard overlay loading/ready selectors, and auth login/register default callbacks resolve into `dashboard-v2` learner destinations without legacy URL normalization.
- Canonical `/dashboard-v2/*` entry navigations still use a root-level dashboard entry bridge when they originate outside the dashboard group, but that bridge is shell-only. The visible content skeleton for the destination route should still come from the route-owned `loading.tsx` inside the mounted `DashboardV2Shell`, not from the root overlay.
...
- creator inventory compatibility for old dashboard URLs was retired during the hard cut once canonical `/dashboard-v2/*` navigation and probes were in place; old query normalization is no longer a runtime responsibility
...
- `src/proxy.ts` now protects only the canonical `/dashboard-v2/*` dashboard family plus `/admin`; it no longer redirects legacy learner/account/creator dashboard URLs into dashboard-v2
...
- the same repo-owned browser-probe layer now also has cold-entry handoff scenarios for `/dashboard-v2/settings`, `/dashboard-v2/membership`, `/dashboard-v2/creator`, `/dashboard-v2/creator/profile`, `/dashboard-v2/creator/settings`, and `/dashboard-v2/creator/payouts`; those checks start from a non-dashboard page, navigate into the target route, and fail if the wrong family/neutral shell scopes are still visible after the target route-ready marker appears. The legacy `/dashboard-v2/creator/settings` scenario now verifies the compatibility redirect handoff into the workspace-ready shell instead of a dedicated settings surface.
...
- `src/app/(dashboard-v2)/dashboard-v2/layout.tsx` and `src/app/admin/layout.tsx` own their active family shell boundaries; the retired `(dashboard)` / `(dashboard-lite)` groups no longer own live dashboard chrome or shell loading
- this route-family boundary rule exists to keep hard refreshes inside the correct dashboard-v2/admin loading family rather than falling back to the global app-level loading UI while layout-level auth/viewer state resolves
- dashboard-v2 family segment `loading.tsx` files must stay content-only because they are rendered inside `DashboardV2Shell` once that layout has resolved; `scripts/check-app-skeletons.mjs` still guards against reintroducing the retired full `DashboardGroupLoadingShell` pattern
- `/dashboard`, `/settings`, `/subscription`, `/dashboard/library`, `/dashboard/purchases`, `/dashboard/downloads`, and old creator dashboard paths no longer exist as dashboard-v2 compatibility entrypoints after the hard cut; current app-facing navigation, probes, and shared menus must target the canonical `/dashboard-v2/*` routes directly
- `src/proxy.ts` now protects the canonical `/dashboard-v2/*` family directly in the same redirect gate as `/admin`; unauthenticated requests to dashboard-v2 learner, account, and creator routes are rejected before route code runs instead of relying on older compatibility entrypoints to catch auth first
- those same dashboard-lite learner routes no longer exist as compatibility handoff entrypoints after the hard cut; when debugging current learner dashboard behavior, treat the dashboard-v2 route family as canonical and do not inspect dashboard-lite pages for active behavior
- the first route-surface cleanup pass is complete: redirect-only learner/account/creator page files under `(dashboard-lite)` and the old creator subtree were deleted after canonical dashboard-v2 parity was established. The later hard-cut pass removed `src/lib/dashboard-route-compatibility.ts`, proxy legacy redirects, overlay/footer/navigation-state legacy normalization, and the `/dashboard/resources` role/access handoff exception.
...
- the old heavier `(dashboard)` route group shell is no longer active after the hard cut; token-first creator/user decisions now belong to canonical dashboard-v2 services and routes instead of legacy dashboard shell wrappers
- `/dashboard-v2/settings` and `/dashboard-v2/membership` are the canonical account routes for current UI and verification; the older `/settings` and `/subscription` surfaces are no longer supported redirect surfaces after the hard cut
...
- `/dashboard-v2/library`, `/dashboard-v2/downloads`, and `/dashboard-v2/purchases` now own the canonical learner dashboard bodies; the older `/dashboard/library`, `/dashboard/downloads`, and `/dashboard/purchases` entries are no longer supported redirect surfaces after the hard cut
...
- dashboard-v2 creator destinations now rely on their route-owned loading surfaces inside `DashboardV2Shell`; they no longer use the retired `DashboardGroupLoadingShell` or root dashboard entry overlay path
- dashboard-v2 phase-2 shell-first loading is now active across the canonical route family, including `/dashboard-v2` and `/dashboard-v2/creator`: overview, learner, account, and the main creator routes now keep route-specific header chrome visible during `loading.tsx` and reserve skeletons for the body sections that still need data, instead of swapping the whole page body to a page-wide route skeleton first
- the creator workspace shell now exposes direct in-app CTA affordances for `Resources`, `Earnings`, and `Payouts` from `/dashboard-v2/creator` itself, so creator revenue history no longer depends on cold-entry-only coverage to prove route handoff behavior
- dashboard-v2 primary navigation now uses the repo-owned `IntentPrefetchLink` path for the sidebar rail, mobile drawer, creator callout CTA, and avatar account-menu destinations instead of bare `next/link`, so canonical learner/account/creator routes can warm on viewport or interaction intent before the click path enters the route family
- `/dashboard-v2/creator/sales` and `/dashboard-v2/creator/payouts` now render their earnings route intro immediately, stream the KPI summary cards first, and then stream the heavier sales ledger / payout tables behind a second in-page suspense branch; the route-ready marker now lands with the summary-card surface instead of waiting for every table row, while the table branch still uses the route-owned earnings loading geometry until it resolves
- `/dashboard-v2/library` and `/dashboard-v2/downloads` currently keep the shell-first loading contract from the dashboard-v2 hardening pass, but their final route bodies still resolve in one pass after route data is ready; the later summary-first runtime perf experiments for those two routes were rolled back, so they are not part of the active perf baseline
- the creator editor routes now use the same shell-first loading contract as the rest of dashboard-v2: `/dashboard-v2/creator/resources/new` and `/dashboard-v2/creator/resources/[id]` no longer render the old `creator-editor` prototype skeleton, and instead keep the real route intro/back-link chrome visible while reserving body space with editor-specific form, delivery, and checklist placeholders
- `/dashboard-v2/creator/apply` now follows the same shell-first loading contract too: the route keeps its real creator-program hero visible during `loading.tsx` and only skeletonizes the approval/application panel beneath it, instead of swapping the full page to the old apply-page skeleton first
- dashboard overlay orchestration now has a narrower source of truth: resources overlays remain in the resources route family, while dashboard-v2 route readiness is cleared by the dashboard-v2 shell and canonical route markers instead of legacy dashboard overlay wrappers
- `DashboardV2Shell` now mounts the shared navigation/overlay readiness helpers itself, so canonical dashboard-v2 learner/account/creator routes can clear family overlays from the shell root as soon as their page-level `data-route-shell-ready` marker appears, without depending on legacy dashboard-lite wrappers to relay readiness
...
- browser-level route coverage now includes `/resources`, top-bar search submit into canonical `/resources?search=...`, canonical search results, no-results recovery, resource detail image rendering, and authenticated preview-image uploader flows on both `/admin/resources/new` and `/dashboard-v2/creator/resources/new`
- the repo-owned smoke Playwright bundle now also includes `tests/e2e/creator-workspace.spec.ts`, and that spec now covers canonical learner cold-entry handoffs (`/dashboard-v2`, `/dashboard-v2/library`, `/dashboard-v2/downloads`, `/dashboard-v2/purchases`, `/dashboard-v2/membership`, `/dashboard-v2/settings`) plus creator workspace/profile/settings/payouts shell-hand-off regressions in `npm run smoke:browser`, `npm run smoke:browser:ci`, and the `browser-smoke.yml` workflow instead of relying only on ad-hoc probes. The cold-entry assertions now also fail if the retired family-neutral scopes (`dashboard-v2-neutral`, `dashboard-v2-creator-neutral`) appear at all during those route transitions.
...
- `/admin/audit` now renders its header/filter shell first and streams the audit table/pagination results through an in-page `Suspense` boundary, while `/dashboard-v2/creator/apply` now keeps route-level loading as the only visible body owner and limits deferred loading to the rejected-feedback subsection instead of streaming the entire status/form panel separately
- repo-owned dashboard verification now covers regular-user creator onboarding too: `tests/e2e/helpers/auth.ts` exports `loginAsUser(...)`, `tests/e2e/creator-workspace.spec.ts` asserts that `/dashboard-v2/creator/apply` clears overlays without shell stacking for non-creators, and `scripts/browser-probe-local.ts` includes a `creator-apply-cold-entry` scenario that fails if dashboard-family shells remain visible after the route-ready marker appears
...
- creator resource create/edit routes now use the same editor-only streaming pattern too: `/dashboard-v2/creator/resources/new` and `/dashboard-v2/creator/resources/[id]` render their page shell first and load category/reference data behind the form fallback instead of blocking the entire editor route on form-data reads
- creator inventory/editor taxonomy reads now go through a cached category helper in `src/services/creator/creator.service.ts`, and the paginated `/dashboard-v2/creator/resources` management path now overlaps category-taxonomy warming with status-summary work before counting/filtering rows; the route still resolves the final table server-side, but it no longer burns a separate uncached category read at each creator-resource surface
- the old `/dashboard/resources` role/access handoff exception was removed in the hard cut; creator apply/resources routing should now enter through canonical dashboard-v2 creator routes directly
- `/dashboard-v2/creator/apply` now uses the same fast-path redirect principle for clearly admitted roles (`ADMIN`/`INSTRUCTOR`) before falling back to `getCreatorAccessState()`, which trims one avoidable creator-access read from the canonical onboarding path without changing the pending/rejected application semantics for regular users
- `/dashboard-v2/creator/apply` now keeps the rejected-state feedback read behind its own nested suspense branch; the rejected shell and reapply form render first, while the optional rejection-reason panel loads separately instead of holding the entire rejected panel
...
- marketplace account dropdowns and dashboard avatar menus now expose canonical `/dashboard-v2/*` targets directly; legacy dashboard URLs are no longer kept as redirect/bookmark compatibility destinations after the hard cut
...
- the dashboard results layer still supports branch-aware fallback geometry on canonical dashboard-v2 learner routes; checkout-return and populated/empty library geometry should be verified against `/dashboard-v2/library` rather than the old `/dashboard/library` alias
