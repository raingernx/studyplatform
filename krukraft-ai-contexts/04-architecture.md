...
- the `/subscription` in-page results fallback now mirrors the final membership hero, summary cards, and billing panel much more closely, so the membership route does not jump between a generic placeholder stack and the final layout when the streamed branch resolves
