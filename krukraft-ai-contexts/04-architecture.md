...
- the dashboard avatar account menu now exposes an explicit probe contract (`data-dashboard-account-trigger`, `data-dashboard-account-menu`, `data-dashboard-account-link`) so browser verification can target the same menu surface the UI owns instead of relying on brittle absolute-position selectors
...
- `/settings` now renders its page shell before auth/profile/preferences data resolve; the route relies on proxy protection for auth entry, then resolves the session and settings payload inside the streamed tabs branch with timeout-aware fallback defaults so the dashboard overlay can clear on shell readiness instead of waiting on the full settings payload
- `/subscription` now follows the same shell-first pattern as `/settings`: the route shell renders immediately, while the membership data branch resolves behind a route-specific `Suspense` fallback instead of blocking the entire top-level page on auth/data work
...
- `/dashboard/library`, `/dashboard/downloads`, and `/dashboard/purchases` now also render their route shell immediately after `requireSession()` and stream the heavy library/download/purchase results sections through route-specific `Suspense` fallbacks instead of blocking the whole dashboard-lite page on purchased-item queries
...
- creator protected analytics, sales, and profile routes now also render their section shell immediately after the creator session gate and stream their heavier KPI/table/form branches behind creator-specific `Suspense` fallbacks instead of blocking the entire workspace route on the first analytics/sales/profile query
- creator protected overview and resource-management routes now also follow the same shell-first pattern: `/dashboard/creator`, `/dashboard/creator/resources`, `/dashboard/creator/resources/new`, and `/dashboard/creator/resources/[id]` render the workspace shell and primary actions first, then stream the heavier KPI/table/editor sections separately so admitted creators no longer wait on the entire workspace payload before seeing the page chrome
...
- `/admin/audit` now renders its header/filter shell first and streams the audit table/pagination results through an in-page `Suspense` boundary, while `/dashboard/creator/apply` now keeps the creator-program header visible and streams the status/form panel separately
